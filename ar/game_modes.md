...
تحتوي PR:BF2 على أكثر من 80 خريطة مع أكثر من 600 نسخة (Layer) مختلفة. يمكن الاطلاع على جميع الخرائط ومعلوماتها التفصيلية عبر: [the PR Map Gallery](http://www.realitymod.com/mapgallery/).

معظم الخرائط تحتوي على 4 طبقات مختلفة تتنوع فيها الأصول والدعم المتاح:

* **Infantry \(16\):** نسخة تعتمد غالبًا على مركبات النقل والمركبات الخفيفة، مع تركيز كبير على قتال المشاة
* **Alternative \(32\):** نسخة بديلة تختلف بشكل كبير أو بسيط عن النسخة الاساسية
* **Standard \(64\):** نسخة الأساسية التي تتضمن معظم الخريطة وجميع الأصول المتاحة، وهي النسخة الافتراضية للعب
* **Large \(128\):** نسخة مصممة لتقديم تجربة مختلفة عن الطبقة الاساسية، غالبًا عبر تغيير الفصائل أو زيادة مدة المعركة
...
في هذا النمط يمكن للاعبين البشر (واحد أو أكثر) القتال إلى جانب أو ضد الذكاء الاصطناعي (البوتات). يُعد هذا النمط مثاليًا للتدرب على المعدات والمركبات قبل استخدامها في معارك حقيقية ضد لاعبين. تم تعديل بعض القواعد والعناصر لتناسب وجود البوتات. معظم الخرائط تحتوي على نسخ خاصة لهذا النمط.

## _المقاومة (Insurgency – INS)_ {#insurgency}

يُعد هذا النمط فريدًا مقارنة ببقية أنماط اللعب في PR. هدف قوات التحالف (BluFor) هو العثور على مخابئ الأسلحة (Weapon Caches) التابعة للمقاومين (RedFor أو OpFor) وتدميرها وهي مخفية في أنحاء الخريطة. يختلف عدد مخابئ الأسلحة المطلوب تدميرها حسب الخريطة والنسخة، وغالبًا يكون هناك 6 مخابئ الأسلحة ويجب تدمير 5 منها للفوز. يفوز المقاومون إذا تمكنوا من حماية ما لا يقل عن  حتى تنفد تذاكر قوات التحالف. بالنسبة لقوات التحالف، تطبق قواعد خسارة التذاكر الخاصة بنمط AAS، بينما لا يمتلك المقاومون تذاكر ويمكنهم إعادة الظهور بلا حدود.
{% figure "../assets/weaponcache.png" %}
مخابئ الأسلحة (Weapon Cache){% endfigure %}

يجب على قوات التحالف جمع المعلومات الاستخباراتية عبر قتل مقاتلي المقاومة أو أسر المدنيين [ألقِ نظرة على مدني](the_civilian.md).لأسر مدني، يجب أن يكون لاعب التحالف على بعد متر واحد من المدني، ويجب استخدام أداة تقييد (أصفاد بلاستيكية)، والتي يمكن الوصول إليها عبر اختيار الرقم 1 في لوحة المفاتيح، أو بديلًا يمكن استخدام بندقية (Shotgun) لمحاكاة التعطيل برصاص غير قاتل.

يفوز التحالف إذا تمكن من تدمير عدد معين من مخابئ الأسلحة قبل نفاد الوقت أو التذاكر، وإلا يفوز المقاومين. يمكن لمهندسي المتمردين إصلاح مخبأ الأسلحة التالفة. يتم عرض عدد مخابئ الأسلحة عند بداية الجولة وعلى الخريطة، كما يمكن للمتمردين معرفة العدد عبر قائمة الكِت/الفرقة (Caps Lock).
...
* قتل مقاوم: +1 نقطة (ضمن 300 متر)
* اعتقال مدني أو مقاوم: +10 نقاط
...
* ![](../assets/unknown%20weapon%20cache.png) **مخبأ أسلحة معروف للمقاومين**: يظهر هذا الرمز على خريطة قوات المقاومين، ويشير إلى أن قوات التحالف قد جمعت معلومات استخباراتية كافية لتحديد موقع المخبأ. بمجرد ظهور هذا الرمز، يتم إنشاء (Spawn) مخبأ الأسلحة فعليًا في ساحة المعركة، ليصبح قابلًا للاستخدام من قِبل المقاومين، كما يصبح هدفًا يمكن لقوات التحالف تدميره. ومع ذلك، توجد فترة سماح مدتها 5 دقائق بين ظهور المخبأ في اللعبة وبين كشف موقعه لقوات التحالف.
* ![](../assets/unknown%20cache.png) **مخبأ غير معروف للمقاومين:** يمثل الموقع الذي سيظهر فيه المخبأ لاحقًا، ولا يظهر إلا للمقاومين لتنظيم الدفاعات.
